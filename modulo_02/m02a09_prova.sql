...
SELECT
	mp.tipo_produto,
	COUNT(DISTINCT(e.entrega_id)) AS num_pedidos
FROM entrega e
JOIN menu_pizzas mp
ON e.pizza_id = mp.pizza_id
GROUP BY 1
ORDER BY 2 DESC
...
SELECT
	rp.forma_pagamento AS tipo_pagamento,
	SUM(e.quantidade*mp.preco) AS valor_total
FROM entrega e
JOIN menu_pizzas mp ON e.pizza_id = mp.pizza_id
JOIN pagamento p ON e.entrega_id = p.entrega_id
JOIN ref_pagamento rp ON p.pagamento_id = rp.pagamento_id
GROUP BY 1
ORDER BY 2
	

-- 7) Monte uma tabela com o nome dos entregadores(as) e a quantidade de pedidos
-- entregues por cada um (soma 47)

SELECT
	re.nome_entregador,
	COUNT(DISTINCT(e.entrega_id)) AS qtd_pedidos
FROM entrega e
JOIN entregador ent ON e.entrega_id = ent.entrega_id
JOIN ref_entregador re ON ent.entregador_id = re.entregador_id
GROUP BY 1
...
SELECT
	mp.nome_produto,
	SUM(e.quantidade) AS quantidade_vendida
FROM entrega e
JOIN menu_pizzas mp ON e.pizza_id = mp.pizza_id
JOIN pagamento p ON e.entrega_id = p.entrega_id
JOIN ref_pagamento rp ON p.pagamento_id = rp.pagamento_id
...
-- ARREDONDE PARA DUAS CASAS DECIMAIS (valor que vai receber pela comissao) é a soma da comissao

SELECT
	t.nome,
	t.tipo_pagamento,
	CASE WHEN t.tipo_pagamento = 'App' THEN ROUND((SUM(t.valor_total)*0.1),2)
		 WHEN t.tipo_pagamento = 'Na Entrega' THEN ROUND((SUM(t.valor_total)*0.05),2) ELSE 0 END AS comissao
FROM (SELECT
			re.nome_entregador AS nome,
			rp.forma_pagamento AS tipo_pagamento,
			sum(e.quantidade*mp.preco) AS valor_total
		FROM entrega e
		JOIN menu_pizzas mp ON e.pizza_id = mp.pizza_id
