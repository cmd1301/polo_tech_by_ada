...
FROM tipo_transacao AS tt
RIGHT JOIN transacoes AS t
ON tt.tipo_transacao_id = t.tipo_transacao_id
GROUP BY 1
ORDER BY 1
...
CREATE TABLE entradas AS
SELECT 
	t.cliente_id,
	sum(t.valor) AS entrada
FROM transacoes AS t
LEFT JOIN tipo_transacao AS tt
ON t.tipo_transacao_id = tt.tipo_transacao_id
WHERE tt.nome_transacao = 'PixIn'
GROUP BY 1
ORDER BY 1

CREATE TABLE saidas AS
SELECT 
	t.cliente_id,
...
LEFT JOIN tipo_transacao AS tt
ON t.tipo_transacao_id = tt.tipo_transacao_id
WHERE tt.nome_transacao = 'PixOut'
GROUP BY 1
ORDER BY 1
...
SELECT 
	...
	sa.saida
FROM saldo AS s
LEFT JOIN entradas AS e ON s.cliente_id = e.cliente_id
LEFT JOIN saidas AS sa ON s.cliente_id = sa.cliente_id
...
SELECT 
	p.pedido_id,
	sum(p.quantidade * prod.valor_unitario) AS valor_total
FROM pedidos AS p
LEFT JOIN produtos AS prod
ON p.produto_id = prod.produto_id
...
SELECT 
	prod.segmento_produto,
	sum(ped.quantidade) AS itens_pedidos
FROM produtos AS prod
LEFT JOIN pedidos AS ped
ON prod.produto_id = ped.produto_id
group BY prod.segmento_produto
ORDER BY 2 DESC
...
SELECT 
	c.cidade,
	count(ped.pedido_id) AS numero_pedidos,
	sum(ped.quantidade * prod.valor_unitario) AS valor_total
FROM clientes AS c
LEFT JOIN pedidos AS ped
...
SELECT 
	prod.nome_produto,
	sum(ped.quantidade) AS num_itens_pedidos
FROM produtos AS prod
LEFT JOIN pedidos AS ped
ON prod.produto_id = ped.produto_id
...
SELECT 
	...
	sum(prod.valor_unitario * ped.quantidade) AS total_vendido
FROM produtos AS prod
LEFT JOIN pedidos AS ped
ON prod.produto_id = ped.produto_id
GROUP BY prod.nome_produto
HAVING sum(ped.quantidade) NOTNULL  
ORDER BY 3 DESC
